...
Interfejsy pozwalające na pozyskiwanie sygnałów bezpośrednio z komórek mózgowych nazywane są inwazyjnymi. W celu \textit{zainstalowania} wymagają one operacji, w której część czaszki jest usuwana, elektroda lub implant jest umieszczany w mózgu, a następnie czaszka jest przytwierdzana z powrotem\cite{bci_introduction}. Technika inwazyjna stosowana jest najczęściej w badaniach przeprowadzanych z wykorzystaniem zwierząt\cite{bci_introduction}. W przypadku ludzi pomiary inwazyjne pobierane są najczęściej w warunkach klinicznych, podczas operacji mózgu lub monitorowania pacjenta bezpośrednio przed operacją.

Interfejsy inwazyjne charakteryzują się najwyższą jakością pozyskanych sygnałów. Spowodowane jest to brakiem zniekształceń sygnałów powodowanych przez czaszkę, która ma 20÷80 krotnie mniejszą przewodność\cite{bci_technology}.
