...
Wykorzystując wewnętrzny mechanizm serializacji zapytanie jest przekształcane do postaci pokazanej na kodzie źródłowym \vref{code:request_sent}, a następnie wysyłane przy użyciu metody \lstinline[language={[Sharp]C}]{SendRequest} pokazanej na kodzie źródłowym \vref{code:send_request}. Odpowiedź na to zapytanie została pokazana na kodzie źródłowym \vref{code:response_received}. Obie wiadomości są tożsame z przykładami zaczerpniętymi z dokumentacji, pokazanymi na kodach źródłowych \vref{code:sample_request} oraz \vref{code:sample_response}.
