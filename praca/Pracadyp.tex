...
\begin{description}
    ...
    gdzie $EEG^{true}_i(t)$ jest \textit{czystym} sygnałem EEG pozyskanym z elektrody $i$ w czasie $t$, $EOG(t)$ jest sygnałem EOG w czasie $t$, a $K$ stałą, która może być estymowana. Posiadając estymację stałej $K$, na podstawie prostego przekształcenia możemy uzyskać estymację czystego sygnału EEG:
    $$ EEG^{true}_i(t) = EEG_i(t) - K \times EOG(t) $$
    Ten sposób usunięcia szumów jest trudniejszy do zastosowania dla artefaktów EMG, ponieważ zakłócenia te pochodzą z wielu źródeł i wymagają opracowania bardziej skomplikowanego modelu.
